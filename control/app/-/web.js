...
(function ($) {
    var $$;
    (function ($$) {
        $$.$checkers_control_checker = () => (Object.assign({ type: '$checkers_control_checker', prop: Object.assign({ '#width': '.em', '#height': '.em' }, $$.$checkers_checker_prop), control: $$.$checkers_checker_content('control') }, $$.$checkers_checker_event, { render: p => {
                $$.$me_atom2_control.pos_prepare(p);
            } }));
    })($$ = $.$$ || ($.$$ = {}));
})($ || ($ = {}));
//checker.js.map
...
(function ($) {
    ...
    (function ($$) {
        $$.$me_panel = () => ({
            type: '$me_panel',
            prop_default: Object.assign({}, $$.$me_atom2_prop_same_def(() => 0, ['borderRadius']), $$.$me_atom2_prop_same_def(() => 'transparent', ['colorBackground']), $$.$me_atom2_prop_cascade(() => 0, 'padding', [
            ...
            render: p => {
                let { ctxWidth, ctxHeight } = $$.$me_atom2_control.pos_prepare(p);
...
(function ($) {
    var $$;
    (function ($$) {
        $$.$checkers_control_circle = () => ({
            type: '$checkers_control_circle',
            prop: Object.assign({ k: () => 1, background: () => 'transparent', border: () => 'transparent', _size: $$.$me_atom2_prop(['.em', '.k'], ({ masters: [em, k] }) => em * k), '#width': '._size', '#height': '._size' }, $$.$me_atom2_pos(2)),
            render: p => {
                const { ctxWidth, ctxHeight } = $$.$me_atom2_control.pos_prepare(p);
                const fillStyle = $$.a('.background');
                let stroke;
                const border = $$.a('.border');
                if (border != 'transparent') {
                    const arr = border.split(/\s+/).filter(s => s != 'solid');
                    const color = arr.filter(s => !s.endsWith('px'));
                    if (!color.length)
                        $$.$me_throw(`${$$.a.curr.name()}: border (${border}) has no color`);
                    const style = color[0];
                    const px = arr.filter(s => s.endsWith('px'));
                    if (!px.length)
                        $$.$me_throw(`${$$.a.curr.name()}: border (${border}) has no 'px'`);
                    const ctxWidth = parseInt(px[0]) * p.pixelRatio;
                    if (Number.isNaN(ctxWidth))
                        $$.$me_throw(`${$$.a.curr.name()}: border (${border}) has no valid 'px'`);
                    stroke = { style, ctxWidth };
                }
                $$.$me_atom2_ctx_circle(Object.assign({}, p, $$.$me_atom2_ctxCenterOf(p), { ctxRadius: $$.$me_atom2_ctxWidthOf(p) / 2, fillStyle,
                    stroke }));
            }
        });
    })($$ = $.$$ || ($.$$ = {}));
...
(function ($) {
    var $$;
    (function ($$) {
        $$.$checkers_control_mark = () => ({
            type: '$checkers_control_mark',
            base: $$.$checkers_control_circle,
            prop: {
                k: () => 0.12,
...
(function ($) {
    ...
    (function ($$) {
        $$.$me_label = () => ({
            type: '$me_label',
            base: $$.$me_panel,
            prop_default: Object.assign({}, $$.$me_atom2_prop_cascade(() => 0, 'align', ['alignHor', 'alignVer'])),
            prop: Object.assign({ _width_text: $$.$me_atom2_prop(['.#ctx', '.text', '/.#pixelRatio'], ({ masters: [ctx, text, pixelRatio] }) => {
            ...
            render: p => {
                ...
                const bottom = p.ctxPos.bottom - ctxPaddingBottom - $$.$me_align_correction($$.a('.alignVer'), () => ctxHeight - ctxFontSize);
                const _left = (ctxTextWidth) => p.ctxPos.left + ctxPaddingLeft + $$.$me_align_correction($$.a('.alignHor'), () => ctxWidth - ctxTextWidth);
...
(function ($) {
    var $$;
    (function ($$) {
        $$.$checkers_control_board = () => ({
            type: '$checkers_control_board',
            prop: {
                '#height': $$.$me_atom2_prop(['.em', '<<.rows'], ({ masters: [em, rows] }) => em * rows.length),
                '#width': $$.$me_atom2_prop(['.em', '<<.cells'], ({ masters: [em, cells] }) => em * cells.length),
            },
            control: {
                cell: $$.$me_atom2_prop({ keys: ['<<.cells', '<<.rows'] }, ({ key: [key_cell, key_row], key_enum: [key_enum_cell, key_enum_row] }) => ({
                    type: '$checkers_control_board_cell_custom',
                    base: $$.$checkers_control_board_cell,
                    prop: {
                        row: () => key_row,
                        rows: () => key_enum_row,
                        cell: () => key_cell,
        ...
        $$.$checkers_control_board_cell = () => ({
            type: '$checkers_control_board_cell',
            prop: Object.assign({ '#isHover': $$.$me_atom2_prop([], () => false), cursor: $$.$me_atom2_prop(['.#isHover', '.mark'], ({ masters: [isHover, mark] }) => isHover && mark ? 'pointer' : 'default', ({ val, atom }) => {
                    $$.$me_atom2_body_cursor({ origin: atom.path, val });
                }), '#width': '.em', '#height': '.em', horOfs: $$.$me_atom2_prop(['.em', '.cells', '.cell'], ({ masters: [em, cells, cell] }) => cells.indexOf(cell) * em), verOfs: $$.$me_atom2_prop(['.em', '.rows', '.row'], ({ masters: [em, rows, row] }) => rows.indexOf(row) * em) }, $$.$me_atom2_pos({
                hor: '.horOfs',
                ver: '.verOfs',
            })),
            control: {
                square: $$.$me_atom2_prop(['.cell', '.row', '.cells', '.rows'], ({ masters: [cell, row, cells, rows] }) => row == '-' || cell == '-' ? null : {
                    type: '$checkers_control_board_cell_square',
                    base: $$.$me_panel,
                    prop: Object.assign({ '#width': '<.#width', '#height': '<.#height' }, $$.$me_atom2_pos(0), { colorBackground: $$.$me_atom2_prop(['<.coord_selected'], ({ masters: [coord_selected] }) => coord_selected === cell + row ? 'red' :
                            (rows.indexOf(row) + cells.indexOf(cell)) % 2 ? 'white' : 'black'), borderWidthLeft: $$.$me_atom2_prop(['<.cell', '<.row'], ({ masters: [cell, row] }) => cell !== 'a' || row === '-' ? 0 : 1), borderWidthRight: $$.$me_atom2_prop(['<.cell', '<.row'], ({ masters: [cell, row] }) => cell !== 'h' || row === '-' ? 0 : 1), borderWidthTop: $$.$me_atom2_prop(['<.cell', '<.row'], ({ masters: [cell, row] }) => row !== '8' || cell === '-' ? 0 : 1), borderWidthBottom: $$.$me_atom2_prop(['<.cell', '<.row'], ({ masters: [cell, row] }) => row !== '1' || cell === '-' ? 0 : 1), colorBorder: () => '/.colorBorder' }),
                }),
                mark: $$.$me_atom2_prop(['.mark'], ({ masters: [mark] }) => !mark ? null : $$.$checkers_control_mark()),
                legend: $$.$me_atom2_prop(['.cell', '.row'], ({ masters: [cell, row] }) => row != '-' && cell != '-' || row == '-' && cell == '-' ? null : {
                    type: '$checkers_control_board_cell_legend',
                    base: $$.$me_label,
                    prop: Object.assign({ '#width': '<.#width', '#height': '<.#height', align: () => 2, text: () => row == '-' ? cell : row }, $$.$me_atom2_pos(0)),
                }),
            },
            event: {
...
(function ($) {
    var $$;
    (function ($$) {
        $$.$checkers_control_win = () => ({
            type: '$checkers_control_win',
            base: $$.$me_label,
            prop: {
                text: $$.$me_atom2_prop(['.player_win'], ({ masters: [player_win] }) => player_win + ' wins'),
                colorBorder: '/.colorMark',
                borderWidth: () => 2,
                padding: $$.$me_atom2_prop(['.em'], ({ masters: [em] }) => em / 50 * 20),
                fontSize: '.em',
                align: () => 2,
                colorBackground: $$.$me_atom2_prop(['.player_win', '/.colorCellWhite', '/.colorCellBlack'], ({ masters: [player, white, black] }) => player === 'white' ? white : black),
                colorText: $$.$me_atom2_prop(['.player_win', '/.colorCellWhite', '/.colorCellBlack'], ({ masters: [player, white, black] }) => player === 'white' ? black : white),
            },
        });
    })($$ = $.$$ || ($.$$ = {}));
})($ || ($ = {}));
...
(function ($) {
    var $$;
    (function ($$) {
        $$.$checkers_control_game = () => ({
            type: '$checkers_control_game',
            dispatch: $$.$checkers_game_dispatch,
            style: $$.$checkers_game_style,
            prop: Object.assign({}, $$.$checkers_game_prop, { '#order': () => ['board_canvas', 'checker_canvas', 'shelf_canvas'] }),
            elem: {
                shelf_canvas: () => ({
                    type: '$checkers_control_game_shelf_canvas',
                    control: {
                        checker: $$.$me_atom2_prop({ keys: ['<.players', '<.checker_keys']
                        }, ({ key: [player, checker] }) => ({
                            type: '$checkers_control_game_shelf_canvas_checker',
                            base: $$.$checkers_control_checker,
                            prop: Object.assign({ '#hidden': $$.$me_atom2_prop([
                                    `<<.checker_anim[${player}][${checker}]`,
                                    `<<.checker_hidden[${player}][${checker}]`
                                ], ({ masters: [anim, hidden] }) => !anim || hidden) }, $$.$checkers_checker_custom_prop(player, checker, 2), $$.$me_atom2_pos({
                                hor: { align: 0, ofs: `<<.checker_left[${player}][${checker}]` },
                                ver: { align: 0, ofs: `<<.checker_top[${player}][${checker}]` },
                            })),
                        })),
                    }
                }),
                checker_canvas: () => ({
                    type: '$checkers_control_game_checker_canvas',
                    control: {
                        checker: $$.$me_atom2_prop({ keys: ['<.players', '<.checker_keys']
                        }, ({ key: [player, checker] }) => ({
                            type: '$checkers_control_game_checker_canvas_checker',
                            base: $$.$checkers_control_checker,
                            prop: Object.assign({ '#hidden': $$.$me_atom2_prop([
                                    `<<.checker_anim[${player}][${checker}]`,
                                    `<<.checker_hidden[${player}][${checker}]`
                ...
                board_canvas: () => ({
                    type: '$checkers_control_game_board_canvas',
                    control: {
                        board: () => ({
                            type: '$checkers_control_game_board_canvas_board',
                            base: $$.$checkers_control_board,
                            prop: Object.assign({ cells: '<<.cells', rows: '<<.rows', coord_mark: '<<.coord_avail', coord_selected: '<<.coord_selected' }, $$.$me_atom2_pos({
                                hor: '<<.boardLeft',
                                ver: '<<.boardTop',
                            })),
                        }),
                ...
                win_canvas: $$.$checkers_win(player_win => ({
                    type: '$checkers_control_game_win_canvas',
                    control: {
                        win: () => ({
                            type: '$checkers_control_game_win_canvas_win',
                            base: $$.$checkers_control_win,
                            prop: Object.assign({ player_win: () => player_win, horOfs: $$.$me_atom2_prop(['<<.boardLeft', '<<.boardWidth', '.#width', '.em'], ({ masters: [boardLeft, boardWidth, width, em] }) => boardLeft + em + (boardWidth - em - width) / 2), verOfs: $$.$me_atom2_prop(['<<.boardTop', '<<.boardHeight', '.#height', '.em'], ({ masters: [boardTop, boardHeight, height, em] }) => boardTop + (boardHeight - em - height) / 2) }, $$.$me_atom2_pos({ hor: '.horOfs', ver: '.verOfs' })),
                        })
                    }
                })),
            },
...
(function ($) {
    var $$;
    (function ($$) {
        $$.$checkers_control_app = (rootElem) => {
            $$.$checkers_settings();
            $$.$me_atom2_elem.prop_default = $$.$me_atom2_control.prop_default =
                {
                    em: '/.em',
                    colorText: '/.colorText',
            ...
            return new $$.$me_atom2_elem({ tail: 'app', cnf: {
                    type: '$checkers_control_app',
                    node: rootElem,
                    elem: {
                        game: () => $$.$checkers_control_game(),
                    },
                    style: {
                        margin: () => 0,
                    },
